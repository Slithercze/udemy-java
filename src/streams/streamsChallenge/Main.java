package streams.streamsChallenge;
...
//        streamG.forEach(System.out::println); doesn't works because pipeline is already used

        System.out.println("------------------------");
        Stream.generate(() -> new Random().nextInt(rSeed, rSeed + 15))//generate if we don't know number range and iterate if we know (for example prime nums)
                .distinct()
                .limit(15)
                .map(i -> "O" + i)
                .sorted()
