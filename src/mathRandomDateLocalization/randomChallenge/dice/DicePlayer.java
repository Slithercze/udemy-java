package mathRandomDateLocalization.randomChallenge.dice;

import mathRandomDateLocalization.randomChallenge.game.GameConsole;
import mathRandomDateLocalization.randomChallenge.game.Player;
...
public class DicePlayer implements Player {
    ...
    public void rollDice() {
        ...
        var newDice = new Random()
                ...
                .sorted()
                .boxed() //converts to Integer from int, alternative is mapToObj(Integer::valueOf)
                .toList();
    ...
    private boolean scoreDice(){
        ...
        String promp = String.join("\n", remainingItems.toArray(new String[0]));
        String userInput = GameConsole.getUserInput(promp + "\n-->  ").toUpperCase();
        if (userInput.isBlank()){
            return false;
        }
