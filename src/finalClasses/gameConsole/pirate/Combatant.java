package finalClasses.gameConsole.pirate;
...
public sealed abstract class Combatant implements Player permits Islander, Pirate, Soldier {
    ...
    boolean useWeapon(Combatant oponent) {

        System.out.println(name + " used the " + currentWeapon);
        if (new Random().nextBoolean()){
            System.out.println(" and HIT *** " + currentWeapon);
            oponent.adjustHealth(-currentWeapon.getHitPoints());
            System.out.printf("%s's health=%d, %s's health=%d%n", name, value("health"), oponent.name(), oponent.value("health"));
            adjustValue("score", 50);
        } else {
            System.out.println(" and missed!");
        }
        return (oponent.value("health") <= 0);
    }
