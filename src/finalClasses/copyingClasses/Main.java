package finalClasses.copyingClasses;
...
//shallow copy        Person[] personsCopy = Arrays.copyOf(persons, persons.length);
        Person[] personsCopy = new Person[5];

        Person[] personCopy2 = persons.clone(); //shallow copy

        for (int i = 0; i < 5; i++) {
            Person current = persons[i];
//            var kids = current.kids(); shallow copy
            var kids = current.kids() == null ? null : Arrays.copyOf(current.kids(), current.kids().length); //deep copy of array inside object
